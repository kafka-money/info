...
export function getPoolLink (token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://cocolending.github.io/swap/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x7874f0fc3f7299f3e0ebbdd7efa8acdde7885880' ? 'FRA' : token0Address}/${'FRA'}`
    )
  } else {
    return (
      `https://cocolending.github.io/swap/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x7874f0fc3f7299f3e0ebbdd7efa8acdde7885880' ? 'FRA' : token0Address}/${token1Address === '0x7874f0fc3f7299f3e0ebbdd7efa8acdde7885880' ? 'FRA' : token1Address
      }`
    )
  }
}
...
export function getSwapLink (token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://cocolending.github.io/swap/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://cocolending.github.io/swap/#/swap?inputCurrency=${token0Address === '0x7874f0fc3f7299f3e0ebbdd7efa8acdde7885880' ? 'FRA' : token0Address
      }&outputCurrency=${token1Address === '0x7874f0fc3f7299f3e0ebbdd7efa8acdde7885880' ? 'FRA' : token1Address}`
  }
}

export function getMiningPoolLink (token0Address) {
  return `https://cocolending.github.io/swap/#/uni/FRA/${token0Address}`
}

export function getUniswapAppLink (linkVariable) {
  let baseUniswapUrl = 'https://cocolending.github.io/swap/#/uni'
  if (!linkVariable) {
    return baseUniswapUrl
  }

  return `${baseUniswapUrl}/FRA/${linkVariable}`
}
...
export const urls = {
  showTransaction: (tx) => `http://blockscout.findorascan.io/tx/${tx}/`,
  showAddress: (address) => `http://blockscout.findorascan.io/address/${address}/`,
  showToken: (address) => `http://blockscout.findorascan.io/token/${address}/`,
  showBlock: (block) => `http://blockscout.findorascan.io/block/${block}/`,
}
